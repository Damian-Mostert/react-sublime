...
const __filepath = (inputDirectory.substring(inputDirectory.lastIndexOf("/"),inputDirectory.length))
...
src: url(".${__filepath}/fonts/${fontName}/${fontFilename}");
}`;
	});
	}
});
...
${readDirectoryAndCombineSync("./node_modules/sublime-styles/")}
...
function readDirectoryAndCombineSync(directoryPath) {
	...
	const files = fs.readdirSync(directoryPath).filter(file=>file!="package.json"&&file!="package.lock.json"&&file!="README.md");
